...
namespace std
{
  inline istream &operator>>(istream &s, tribool::tribool &a)
  {
    char c;
    s >> c;
    a = (c == '1') ? tribool::tribool::true_ : (c == '0') ? tribool::tribool::false_ : tribool::tribool::other;
    return s;
  }
  inline ostream &operator<<(ostream &s, tribool::tribool a)
  {
    char c = (a == tribool::tribool::true_) ? '1' : (a == tribool::tribool::false_) ? '0' : '?';
    return s << c;
  }
}
