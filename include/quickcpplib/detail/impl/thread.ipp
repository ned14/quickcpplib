    ...
    namespace detail
    {
      ...
      template <class T> class atomic_page_memory_storage_ptr
      {
        ...
        union _storage_union
        {
          ...
          {
            algorithm::memory::cmemset(nonatomic._storage, byte::to_byte(0), sizeof(nonatomic._storage));
          }
      ...
      struct instance_storage_segment
      {
        ...
        instance_storage_segment(size_t pagesize) noexcept
            : storage_maximum(uint16_t((pagesize - offsetof(instance_storage_segment, _storage)) / sizeof(value_type)))
        {
        }
